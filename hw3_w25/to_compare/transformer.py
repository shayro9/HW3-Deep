...
def get_attention_indices(seq_len, w):
    ...
    # Calculate the distance between each pair of row and column indices
    distance = torch.abs(rows_idx - column_indices)

    # Create a valid mask where the distance is within the window size
    valid_mask = distance <= (w // 2)

    # Apply the mask to get the valid row and column indices
...
def sliding_window_attention(q, k, v, window_size, padding_mask=None):
    ...
    # Using only 4 dim tensors
    # 3 dim >> only one head >> heads_dim = 1
    if len(q.shape) == 3:
    ...
    similarities = torch.full((batch_size, heads_dim, seq_len, seq_len), minus_inf).to(device)

    # Apply the mask to get the valid row and column indices
    row_indices, column_indices = get_attention_indices(seq_len, window_size)
    dot_products = q[:, :, row_indices, :] * k[:, :, column_indices, :]
    similarities[:, :, row_indices, column_indices] = torch.sum(dot_products, -1)
    ...
    if padding_mask is not None:
        ...
        similarities = similarities.masked_fill_(combined_padding == 0, minus_inf)
        similarities = similarities.to(dtype=torch.float, device=device)

    attention = similarities / (embed_dim ** 0.5)
